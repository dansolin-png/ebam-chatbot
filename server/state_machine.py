...
async def process_message(
    ...
    if state_type == "choice":
        ...
        if matched_option:
            ...
            # Mode: custom static reply
            elif mode == "static":
                static_message = opt_cfg.get("static_message", "")
                next_state_id = opt_cfg.get("next") or transitions.get(matched_option, state.get("fallback", "start"))
                next_state = get_state(flow, next_state_id)
                bot_message = static_message
                if next_state.get("message"):
                    bot_message = (bot_message + "\n\n" + next_state["message"]) if bot_message else next_state["message"]
                return {
                    "next_state_id": next_state_id,
                    "bot_message": bot_message,
                    "bot_options": next_state.get("options"),
