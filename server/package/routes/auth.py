...
def _admin_display_name() -> str:
    row = db.get_admin_user(ADMIN_USERNAME)
    return (row or {}).get("display_name") or "Admin"
...
def login(req: LoginRequest):
    ...
    if user and _check_password(req.password, user["password_hash"]):
        return {
            ...
            "display_name": user.get("display_name") or req.username,
        }
...
def get_me(payload: dict = Depends(get_current_user)):
    ...
    if username == ADMIN_USERNAME:
        return {"username": username, "role": "admin", "display_name": _admin_display_name()}
...
def update_me(req: UpdateProfileRequest, payload: dict = Depends(get_current_user)):
    ...
    if username == ADMIN_USERNAME:
        # Upsert a minimal row for admin so display_name can be stored
        existing = db.get_admin_user(ADMIN_USERNAME)
        if not existing:
            db.tbl_admin_users.put_item(Item={"username": ADMIN_USERNAME, "display_name": req.display_name.strip()})
        else:
            db.update_admin_user_display_name(ADMIN_USERNAME, req.display_name.strip())
    else:
        db.update_admin_user_display_name(username, req.display_name.strip())
    return {"username": username, "display_name": req.display_name.strip()}
